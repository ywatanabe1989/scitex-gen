...
An empty-ish conftest.py at tests/ is the canonical SciTeX
convention (audit-project PS208) — it pins the pytest
rootdir and gives downstream fixtures a home.

It also performs module-import-time coverage wiring (parallel +
subprocess support). `os.environ.setdefault` would be a no-op here
because pytest-cov has already set COVERAGE_FILE to a tmp dir by the
time conftest is loaded — so the assignment must be a force-set.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_gen_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
