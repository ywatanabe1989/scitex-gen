...
| find_closest, isclose, is_nan            | scitex_math                 |
...
| is_defined_global, is_defined_local,     | scitex_introspect           |
| is_later_or_equal, this_file, THIS_FILE  | scitex_introspect           |
| title2path                               | scitex_path                 |
| copy_files, _copy_a_file, copy_the_file  | scitex_path                 |
| var_info                                 | scitex_types                |
| is_ipython, is_script                    | scitex_context              |
| embed, less, paste                       | scitex_repl  (new repo)     |
| cache, alternate_kwarg, partial_at       | scitex_decorators           |
| describe                                 | scitex_stats (describe_pandas) |
| unique, uq                               | scitex_pd                   |
| ThreadWithReturnValue, wait_key,         | scitex_parallel             |
| _return_counting_process                 | scitex_parallel             |
| TimeStamper                              | scitex_datetime             |

Removed entirely (no replacement; were already deprecated forwarders):

- ``start``, ``close``, ``running2finished``

Removed entirely (the destination peer already had a better
implementation; no semantic-preserving port was warranted):
