...
try:
    ...
    def nanstd(x, dim=None, keepdim=False, unbiased=True):
        ...
        mask = ~_torch.isnan(x)
        n = mask.sum(dim=dim, keepdim=keepdim) if dim is not None else mask.sum()
        x = _torch.where(mask, x, _torch.zeros_like(x))
        mean = x.sum(dim=dim, keepdim=True) / n.clamp(min=1)
        var = ((x - mean) ** 2 * mask).sum(dim=dim, keepdim=keepdim) / (
            n - (1 if unbiased else 0)
        ).clamp(min=1)
        return var.sqrt()
